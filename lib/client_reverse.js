...
wst_client_reverse.prototype.start = function(portTunnel, wsHostUrl, remoteAddr, uuid) {
  ...
  //Getting paramiters
  let url = require("url");
  const urlWsHostObj = url.parse(wsHostUrl);
  const _ref1 = remoteAddr.split(":"), remoteHost = _ref1[0], remotePort = _ref1[1];
...
function tcpConnection(wsConn, host, port){
  ...
  const tcpConn = net.connect({port: port, host: host}, function () {
  });
