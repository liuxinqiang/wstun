...
let allow_list, allow_list_file;

const eventEmitter = require('events').EventEmitter;
eventEmitter.prototype._maxListeners = 1000;

const newWSTCP_DATA = new eventEmitter();
  ...
  this.wsServerForControll.on('request', (function(_this){
    return function(request){
      ...
      request.tcpServer = new net.createServer();
        ...
        const src_address = request.httpRequest.client._peername.address.split(":")[3];

        if (uri.query.dst != undefined){

          const remoteAddr = uri.query.dst;
          const client_uuid = uri.query.uuid;
          ref1 = remoteAddr.split(":");
          const portTcp = ref1[1];
        ...
        if(allow_list_file != undefined ){
            if(client_uuid != undefined){
                ...
                const c = allow_list.filter(it => it.client === client_uuid);
                if (c.length > 0){
